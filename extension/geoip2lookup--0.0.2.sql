...
language sql as
$$
select raw_geoip2_json($1, $2, current_setting('geoip2lookup.path'));
...
$$ SELECT isp($1, current_setting('geoip2lookup.path')); $$ SET search_path FROM CURRENT;
...
LANGUAGE SQL AS
$$ SELECT connection_type($1, current_setting('geoip2lookup.path')); $$ SET search_path FROM CURRENT;
...
LANGUAGE SQL AS
$$ SELECT anonymous_ip($1, current_setting('geoip2lookup.path')); $$ SET search_path FROM CURRENT;
...
LANGUAGE SQL AS
$$ SELECT city($1, current_setting('geoip2lookup.path'), current_setting('geoip2lookup.language')); $$ SET search_path FROM CURRENT;
...
LANGUAGE SQL AS
$$ SELECT country($1, current_setting('geoip2lookup.path'), current_setting('geoip2lookup.language')); $$ SET search_path FROM CURRENT;
